...
var id_activity_string = params.get('id');
...
function initBrowsing() {
    ...
    $.getJSON("http://"+JAVA_TOMCAT_HOST+ "/MaintenanceProject/Planner/jsp/visualize_activity.jsp",{id_activity: id_activity_string}, function (data) {
        renderGui(data);
    }).fail(function () {
    });
    ...
    document.getElementById("createButton").onclick=modifyActivity;
...
function renderGui(data) {
    ...
    let inputPlanner = document.getElementById("inputPlanner");
	let inputDescription = document.getElementById("inputDescription");
	let inputInterruptible = document.getElementById("inputInterruptible");
    ...
    let inputProcedure = document.getElementById("inputProcedure");
    let inputEWO = document.getElementById("inputEWO");

    inputPlanner.value=data[0].id_user;
	inputDescription.value=data[0].description;
	if(data[0].interruptible === "true"){
        inputInterruptible.checked = true;
    }
    else{
        inputInterruptible.checked = false;
    }

	inputWeek.value=data[0].week;
	inputTime.value=data[0].eit;
	inputSite.value=data[0].area;
    inputTypology.value=data[0].typology;
    inputProcedure.value = data[0].name_procedure;
    if(data[0].ewo === "true"){
        inputEWO.checked = true;
    }
    else{
        inputEWO.checked = false;
    }
...
function modifyActivity() {
	...
	$.ajax({
        type: "POST",
        url: "http://"+JAVA_TOMCAT_HOST+ "/MaintenanceProject/Planner/jsp/modify_activity.jsp",
        data: {id_activity: id_activity_string, description: description_string, week: week_string, interruptible: interruptible_string, eit: eit_string, 
