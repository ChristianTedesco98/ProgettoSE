...
function initBrowsing() {
    ...
    $.getJSON("http://"+JAVA_TOMCAT_HOST+ "/MaintenanceProject/DBLoader/jsp/visualize_assigned_competencies.jsp", {id_procedure: myParam}, function (data) {
        renderGui(data);
    }).fail(function () {
    });
...
function assign(){
    ...
    $.ajax({
        type: "POST",
        url: "http://"+JAVA_TOMCAT_HOST+ "/MaintenanceProject/DBLoader/jsp/assign_competencies.jsp",
        data: {id_procedure: myParam, num_competencies: checks.length, checks: checks.toString(), ids: ids.toString()},
        dataType: "html",
